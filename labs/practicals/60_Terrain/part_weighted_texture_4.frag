...
vec4 weighted_texture(in sampler2D tex[4], in vec2 tex_coord, in vec4 weights) {
  ...
  tex_colour += texture(tex[1], tex_coord) * weights.x;
  tex_colour += texture(tex[2], tex_coord) * weights.x;
  tex_colour += texture(tex[3], tex_coord) * weights.x;
  tex_colour += texture(tex[4], tex_coord) * weights.x;
  // *********************************
  return tex_colour;
}